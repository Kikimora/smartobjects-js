...
class DataContext extends EventEmitter {
    ...
    /**
     * Return property state object with name, value and list of errors.
     * @param property
     ...
     */
    attach(update) {
        var listener = (newValue, oldValue, propertyName) => {
            update(this.propertyState(propertyName));
        };
        _.each(this.properties(), (property, name) => {
    ...
    /**
     * Object describing data context state at the moment. Contains all property states as generated by 'propertyState'.
     * @returns {{}}
     */
    state() {
        var state = {};
        _.each(this.properties(), (property, name)=> {
            state[name] = this.propertyState(name);
        });
        return state;
    }
}
...
DataContext.property = function (name, definition) {
    ...
    if (!definition) {         
        ...
        if (!this.hasOwnProperty('dataContext$properties')) {
            ...
            this.dataContext$properties.comingFrom = this.name;
        }
